...
# docker run -d -p 80:80 -e RAILS_MASTER_KEY=<value from config/master.key> --name friendlyrubyblog friendlyrubyblog
...
RUN apt-get update -qq && \
    apt-get install --no-install-recommends -y git make cmake python3 xz-utils build-essential libglfw3-dev libgl-dev libxext-dev && \
    rm -rf /var/lib/apt/lists /var/cache/apt/archives
